...
def parse_flirt_file(f):
 ...
 if header.features & FlirtFeatureFlag.FEATURE_COMPRESSED:
  ...
  f_read = f.read()
  try :
    f = StringIO(zlib.decompress(f_read))  # Untested
  except :
    try:
      # Manual mode
      print("[-] cannot find IDA FLIRT signature.. starting manual-mode with binwalk (you need to install it if not already)")
      offset_from = 0
      p = Popen(['binwalk', f.name], stdout=PIPE, stderr=PIPE)
      output, err = p.communicate()
      for tryz in [re.sub(r"[ ][ ]+"," ",x) for x in output.split("\n") if "Zlib compressed data" in x]:
        offset_from = int(tryz.split(" ")[0])
      f_read = open(f.name, "r").read()
      f = StringIO(zlib.decompress(f_read[offset_from:]))
    except :
      raise Exception("Cannot find zlib compressed file even with binwalk.. try to install it, otherwise check your file")
